...
public abstract class Projectile implements Skill, Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 3803228292065635098L;
	...
	public Projectile(int positionX, int positionY, int width, int height, int damage, Enemy e, Player player) {
		...
		if (active) {
			...
			if (animation != null) {
				...
				if (direction == DIRECTION.UP)
					batch.draw(imageUp, positionX, positionY, width, height);
				if (direction == DIRECTION.DOWN)
					batch.draw(imageDown, positionX, positionY, width, height);
				if (direction == DIRECTION.LEFT)
					batch.draw(imageLeft, positionX, positionY, width, height);
				if (direction == DIRECTION.RIGHT)
					batch.draw(imageRight, positionX, positionY, width, height);
			}
		}
